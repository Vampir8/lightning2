...
QtpMain::QtpMain()
{
    ...
    _view->setBackgroundBrush(QBrush(QColor(58,58,58), Qt::SolidPattern));
    ...
    setWindowTitle(tr("Lightning 2.0"));
    setUnifiedTitleAndToolBarOnMac(true);
}

void QtpMain::load(QDataStream &ds)
{
    _diagram->clear();
...
void QtpMain::save(QDataStream &ds)
{
    foreach(QGraphicsItem *item, _diagram->items())
        if (item->type() == QtpComp::Type)
        {
            ds << item->type();
            ((QtpComp*) item)->save(ds);
        }

    /*foreach(QGraphicsItem *item, _view->items())
        if (item->type() == QtpPin::Type)
        {
            ds << item->type();
...
void QtpMain::registerComponent(QtpComp::CompInfo const& compInfo)
{
    ...
    _toolBox->addItem(_compWidget, tr("Component"));
...
void QtpMain::createToolBox()
{
    ...
    //_toolBox->setMinimumHeight(100);
    //_toolBox->setMaximumWidth(100);
